...
public class PaymentService {
    ...
    public CreatePaymentOrderResponse createPaymentOrder() {
        ...
        order.setCurrency(razorpayService.getCurrency());
    ...
    public String verifyPayment(VerifyPaymentRequest request) {
        ...
        PurchaseOrder order = purchaseOrderRepository.findById(request.localOrderId())
        ...
        if (!validSignature) {
            ...
            failedTx.setOrderId(order.getId());
            failedTx.setRazorpayPaymentId(request.razorpayPaymentId());
            failedTx.setRazorpayOrderId(request.razorpayOrderId());
            failedTx.setRazorpaySignature(request.razorpaySignature());
        ...
        successTx.setOrderId(order.getId());
        successTx.setRazorpayPaymentId(request.razorpayPaymentId());
        successTx.setRazorpayOrderId(request.razorpayOrderId());
        successTx.setRazorpaySignature(request.razorpaySignature());
        successTx.setStatus("SUCCESS");
        paymentTransactionRepository.save(successTx);
